...
function agregarCombinacion() {
    ...
    combinacion.innerHTML = `
        ...
        <input type="number" class="combinacion-precio" required min="0" step="0.01" placeholder="Precio">
...
function agregarProducto(event) {
    ...
    document.querySelectorAll('.combinacion-item').forEach(item => {
        ...
        const precio = parseFloat(item.querySelector('.combinacion-precio').value);
        const cantidad = parseInt(item.querySelector('.combinacion-cantidad').value);
        
        if (talla && color && precio >= 0 && cantidad >= 0) {
            combinaciones.push({ talla, color, precio, cantidad });
        }
    });
...
function actualizarVistaPrevia() {
    ...
    document.querySelectorAll('.combinacion-item').forEach(item => {
        const talla = item.querySelector('.combinacion-talla').value || '-';
        const color = item.querySelector('.combinacion-color').value || '-';
        const precio = item.querySelector('.combinacion-precio').value || '0.00';
        const cantidad = item.querySelector('.combinacion-cantidad').value || '0';
        
        if (talla !== '-' || color !== '-' || precio !== '0.00' || cantidad !== '0') {
            combinacionesPreview.innerHTML += `
                <div class="combinacion-preview">
                    Talla: ${talla} | Color: ${color} | Precio: S/. ${precio} | Cantidad: ${cantidad}
                </div>
            `;
        }
    });
    ...
    const modalContent = `
        <form id="form-editar-producto" onsubmit="guardarEdicion(${index}, event)">
            ...
            <div class="grupo-doble">
                ...
                <div class="grupo-formulario">
                    <label>Marca</label>
                    <select id="edit-marca" required>
                        ${datos.marcas.map(marca => 
                            `<option value="${marca}" ${producto.marca.toLowerCase() === marca.toLowerCase() ? 'selected' : ''}>${marca}</option>`
                        ).join('')}
                    </select>
                </div>
            </div>
...
function mostrarMarcas() {
    ...
    datos.marcas.forEach((marca, index) => {
        lista.innerHTML += `
            <div class="item">
                <span>${marca}</span>
                <button class="btn-eliminar" onclick="eliminarMarca(${index})">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
        `;
...
function agregarMarca() {
    const nuevaMarca = document.getElementById('nueva-marca').value.trim();
    const nuevaMarcaLower = nuevaMarca.toLowerCase();
    
    if (!nuevaMarca) {
        mostrarNotificacion('Ingrese un nombre de marca', 'error');
        return;
    }
    
    const existe = datos.marcas.some(marca => marca.toLowerCase() === nuevaMarcaLower);
    
    if (!existe) {
        datos.marcas.push(nuevaMarca);
        guardarDatos();
        mostrarMarcas();
        actualizarSelectores();
        document.getElementById('nueva-marca').value = '';
        mostrarNotificacion('Marca agregada correctamente', 'exito');
    } else {
        mostrarNotificacion('Esta marca ya existe', 'error');
    }
}

function eliminarMarca(index) {
    if (confirm('¿Eliminar esta marca?')) {
        datos.marcas.splice(index, 1);
        guardarDatos();
        mostrarMarcas();
        actualizarSelectores();
        mostrarNotificacion('Marca eliminada', 'exito');
    }
}
